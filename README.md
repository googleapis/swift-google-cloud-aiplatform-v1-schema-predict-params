...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-aiplatform-v1-schema-predict-params.git --from 0.3.0
```

Then add `GoogleCloudAIPlatformV1SchemaPredictParams` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudAIPlatformV1SchemaPredictParams <target-name> --package swift-google-cloud-aiplatform-v1-schema-predict-params
```
